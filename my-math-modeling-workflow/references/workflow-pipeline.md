...
工作流目录：D:\Math Modeling\my-math-modeling-workflow
具体赛题项目目录：D:\Math Modeling\projects\2026-国赛-A
...
给 Codex：
```text
DeepSeek 已完成实现。请审查项目目录：D:\Math Modeling\projects\2026-国赛-A
...
核心原则：DeepSeek 必须同时扮演"分析者、实现者、审稿人"，每一步都要强制产出文件和自检报告。
...
现在只做阶段 1：读取赛题和数据，生成 PROBLEM_ANALYSIS.md。完成后暂停。
```

### 2-7. 各阶段

各阶段的提示词和操作步骤与模式 A 的阶段 1-6 一致，区别在于 DeepSeek 需要同时完成分析和实现。详细阶段定义见 `workflow-blueprint.md`。
...
纯 DeepSeek 工作流最大风险是"自己写、自己信、自己过"。必须额外注意：
- 每阶段停顿一次，人工快速看一遍。
...
- **时间紧、题目难、需要更稳** → 用 Codex + DeepSeek（模式 A）
- **只有一个模型可用、题目较简单** → 用纯 DeepSeek（模式 B）
